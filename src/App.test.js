...
import { render, waitFor, wait, cleanup } from "@testing-library/react";
...
test("render App without errors", async () => {
  await mockFetchShow.mockResolvedValueOnce(mockDataShow);
  render(<App />);
});

test("renders fetching data while show array is empty", async () => {
  mockFetchShow.mockResolvedValueOnce(mockDataShow);
...
test("selectes seasons from dropbown", async () => {
  mockFetchShow.mockResolvedValueOnce(mockDataShow);
